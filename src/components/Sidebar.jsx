    ...
    return (
        ...
        <div className='ml-3 h-screen md:overflow-hidden overflow-auto md:hover:overflow-auto pb-10 '>
            { activeMenu && (<> <div className='flex justify-between items-center'>
                <Link to='/' onClick={ handleCloseSidebar } className='items-center gap-3 ml-3 mt-4 flex text-xl font-extrabold tracking-tight dark:text-light text-slate-900'>
                    <SiShopware className='dark:text-cyan-500' /><span className='dark:text-cyan-400' >Shoppy </span>
                </Link>
                    ...
                    { links.map((item) => (
                        <div key={ item.title }>
                            ...
                            { item.links.map((link) => (
                                ...
                                > { link.icon } <span className='capitalize'>{ link.name } </span> </NavLink>
                            )) }
                        </div>
                    ))
                    }
